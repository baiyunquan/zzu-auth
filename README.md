...
## 链路状态联动与自愈看门狗 (zzu-link-watchdog)

针对校园网高并发场景下交换机惩罚断流痛点，项目内置了全自动的硬件级链路联动看门狗。

### 痛点根因：校园网交换机 300 秒（5分钟）惩罚性阻断

在校园网中，如果上游设备（如京东云无线宝 PCDN、多连接 BT/高并发下载）产生了瞬间突发流量或 UDP 洪峰：
1. **交换机风暴控制触发**：接入层交换机（锐捷/华为）的 `storm-control` 或 `errdisable` 机制会将墙孔端口置于**惩罚阻断状态**，默认恢复周期为 **300 秒（整整 5 分钟）**；
2. **人工插拔的本质**：拔下网线时交换机检测到 `Link Down`，硬件 ASIC 芯片会立刻清空端口惩罚锁存器（Clear Error Latch），重新插上即刻恢复；
3. **AP 级联链路感知盲区**：若上游接有 AP/交换机（如雅典娜 AP 模式），当墙孔断流时，AP 到下游主路由（如 MT7621）的网线依然是物理通的（Link UP），下游主路由网卡无法感知上游死亡，导致网络彻底假死且无法自愈。
...
│ 京东云雅典娜 (AP 网桥模式)                             │
│                                                        │
│  [ zzu-link-watchdog 自愈守护进程 ]                    │
│      │                                                 │
│      ├─► 周期探测校园网 Portal / 网关 (172.16.2.9:801)  │
│      │                                                 │
│      ▼ (连续 3 次失败 / 假死检测)                       │
│  1. 软件闪断 eth4 (down 1s -> up):                     │
│     物理断开与交换机连接，强制清除交换机 ASIC 惩罚锁存器 │
│                                                        │
│  2. 同步闪断 eth0 (down 1s -> up):                     │
│     透传 Carrier Down 信号给下游主路由                  │
└─────────────────────────┬──────────────────────────────┘
                          │ (eth0)
                          ▼
┌────────────────────────────────────────────────────────┐
│ MT7621 (ImmortalWrt / HarmonyOS 主路由)                │
│                                                        │
│  WAN 网卡感知到物理插拔 (Carrier Down -> Up)           │
│  ├── 触发 netifd 快速刷新 DHCP 租约与 ARP 缓存         │
│  └── 触发 zzu-auth 守护进程立即发起毫秒级重新认证      │
└────────────────────────────────────────────────────────┘
```

### 使用方法

#### 1. 独立运行（适用于任何 Linux / 官方固件）

```sh
# 赋予执行权限
chmod +x scripts/zzu-link-watchdog.sh

# 测试当前网络与接口识别
./scripts/zzu-link-watchdog.sh --test

# 后台守护运行（默认自动探测 eth4 / eth0）
./scripts/zzu-link-watchdog.sh -D

# 显式指定上游为 eth4，下游为 eth0
./scripts/zzu-link-watchdog.sh -u eth4 -d eth0 -D
```

#### 2. OpenWrt / ImmortalWrt 服务化管理

在 OpenWrt 系统上已由 procd 深度托管，支持随系统自启：

```sh
# 启用并启动服务
/etc/init.d/zzu-link-watchdog enable
/etc/init.d/zzu-link-watchdog start

# 查看看门狗实时审计日志
logread -e zzu-watchdog
```
